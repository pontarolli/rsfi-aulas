...
```{mermaid}
flowchart TB
  A((A)) -->|"transmite"| AP["Access Point"]
  B((B)) -->|"transmite → colisão"| AP
  C((C)) -.->|"aguarda"| AP
```
...
```{mermaid}
flowchart LR
  subgraph CD["CSMA/CD: detecta colisão"]
    direction TB
    HA["Host A"] --> COL{{"colisão"}}
    HB["Host B"] --> COL
    COL --> RET["detecta o erro\ne retransmite"]
  end
  subgraph CA["CSMA/CA: evita colisão"]
    direction TB
    A2["A"] -->|"escuta"| AP2["AP"]
    B2["B"] -.->|"backoff"| AP2
  end
...
```{mermaid}
flowchart TB
  subgraph Canal["um envio do AP, vários clientes"]
    direction TB
    RU4["RU — Cliente 4"]
    RU3["RU — Cliente 3"]
    RU2["RU — Cliente 2"]
...
```{mermaid}
flowchart LR
  A((A)) -->|"RTS"| AP["Access Point"]
  AP -->|"CTS"| B((B))
```

*A e B não se escutam diretamente, mas ambos alcançam o mesmo AP.*
...
*Um usuário por slot de tempo, em sequência.* Cada usuário transmite apenas no seu horário; enquanto um fala, os outros aguardam. **Exemplos:** redes celulares legadas, rádio digital e redes industriais sem fio.

### FDMA — acesso por divisão de frequência

```{mermaid}
flowchart TB
  subgraph Espectro["faixas paralelas de frequência"]
    direction TB
    F4["Faixa 4 — U4"]
    F3["Faixa 3 — U3"]
    F2["Faixa 2 — U2"]
...
```{mermaid}
flowchart LR
  U1((U1)) -->|"código A"| REC["Receptor"]
  U2((U2)) -->|"código B"| REC
  U3((U3)) -->|"código C"| REC
```

*Mesmo tempo + mesma frequência, códigos diferentes.* Usuários compartilham tempo e frequência, mas cada transmissão usa um **código diferente**; o receptor usa o código correto para separar o sinal desejado — os demais sinais aparecem como interferência controlada. **Exemplos:** redes 3G, GPS e sistemas com espalhamento espectral.
